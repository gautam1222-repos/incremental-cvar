...
def inc_var_cvar_optimization(alpha, var_step_length, GCVaR_step_length, params_step_length, epochs):
    ...
    for t in range(epochs):
        ...
        if (t+1)%100 == 0:
            file_cnt += 1
            text_file = open("Grad"+str(file_cnt)+"_without_bound.txt", "w")
        ...
        txt_file = open("grad_iteration_num_without_bound.txt","w")
    ...
    text_file = open("Grads_Final_1_without_bound.txt", "w")
